...
enum {
    ...
    PANEL_REC_NOTIFY, PANEL_REC_ABOUT, PANEL_REC_LOGOUT,
...
static LRESULT CALLBACK SettingsProc(HWND h,UINT m,WPARAM w,LPARAM l){
    ...
    case WM_COMMAND:{
        ...
        case IDC_PANEL_BASE+50:    // logout
            ...
            return 0;
        case IDC_PANEL_BASE+60: DestroyWindow(h); PrintDesigner_Open(sw->hMain); return 0;
        case IDC_PANEL_BASE+61: DestroyWindow(h); RestoreDesign_Open(sw->hMain); return 0;
        case IDC_PANEL_BASE+62: DestroyWindow(h); OpenProfileRequestsInbox(sw->hMain); return 0;
        case IDC_PANEL_BASE+63: DestroyWindow(h); OpenBackupLogViewer(sw->hMain); return 0;
...
static void openSettingsWindow(HWND hMain,const User& u,bool admin){
    ...
    sw->user=u; sw->isAdmin=admin; sw->active=0; sw->hMain=hMain;
    sw->navW=admin?S(220):S(180);
    if(admin){
    ...
    int W=admin?S(1000):S(760), H=admin?S(640):S(520);
...
// ---------------------------------------------------------------- public -----
void OpenReceptionSettings(HWND hMain, const User& u){ openSettingsWindow(hMain,u,false); }
void OpenManagementSettings(HWND hMain, const User& u){ openSettingsWindow(hMain,u,true); }
void OpenSettings(HWND hMain, const User& u){
    if(u.role==ROLE_ADMIN) OpenManagementSettings(hMain,u);
