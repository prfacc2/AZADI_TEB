...
struct NetSyncCfg {
    ...
    std::wstring share_path;   // e.g. \\NAS01\DarmanPlus_Share
    NetSyncCfg():enabled(false){}
};
NetSyncCfg NetSync_Config();

// POST a UTF-8 JSON body to <admin_host><path>. Falls back to writing a file
// in share_path\DarmanPlus\outbox when the host is unreachable. Returns true if
...
// next file from share_path\DarmanPlus\inbox. Returns true if something was read.
