...
static std::wstring pdReceiptBarcode(long long seed){
    if(seed<=0) return L"";
    long long v = 1000000LL + ((seed * 4649LL) % 9000000LL);
    wchar_t b[16]; swprintf(b,16,L"%07lld",v);
    return b;
}
...
// The canonical receipt number for a record: the explicitly-assigned receiptNo
// when present, otherwise today's queue number (which is what the legacy
// {receiptNo} token has always printed).
static long long pdReceiptSeed(const ReceptionRecord& r){
    if(r.receiptNo > 0) return r.receiptNo;
    return r.queueNo > 0 ? (long long)r.queueNo : 0;
}
